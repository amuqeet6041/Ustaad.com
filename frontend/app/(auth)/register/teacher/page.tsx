...
export default function TeacherRegisterPage() {
  ...
  const validate = (): Errors => {
    ...
    if (!acceptedTerms) nextErrors.terms = "You must agree to the Terms of Service.";
    return nextErrors;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      storeAuth(data);
      router.push(data.user?.role === "student" ? "/student-dashboard/overview" : "/teacher-dashboard/overview");
    } catch {
    ...
    <AuthLayout>
      <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-7 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-9">
        ...
        <p className="mt-2 text-sm text-white/60">
          Share your expertise and start teaching.
        </p>
        ...
        <form onSubmit={handleSubmit} noValidate className="mt-7 space-y-5">
          <div>
            ...
            />
            {errors.fullName && (
              <p id="fullName-error" className="mt-1.5 text-xs font-medium text-[#9C3B3B]">
                {errors.fullName}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.confirmPassword && (
              <p id="confirmPassword-error" className="mt-1.5 text-xs font-medium text-[#9C3B3B]">
                {errors.confirmPassword}
              </p>
            )}
          </div>
          ...
          <div>
            <label className="flex items-start gap-3 text-sm text-white/70">
              <input
                ...
                className="mt-0.5 h-4 w-4 shrink-0 rounded border-white/20 bg-[#0D172B] accent-[#B5651D]"
              />
              <span>
                I agree to the{" "}
                <a href="#" className="font-medium text-[#D18A4A] transition hover:text-[#E5A15F]">
                  Terms of Service
                </a>{" "}
                and{" "}
                <a href="#" className="font-medium text-[#D18A4A] transition hover:text-[#E5A15F]">
            ...
            {errors.terms && (
              <p id="terms-error" className="mt-1.5 text-xs font-medium text-[#9C3B3B]">
                {errors.terms}
              </p>
            )}
          </div>
          ...
          >
            {loading ? "Please wait..." : "Create Ustaad Account"}
          </button>
        </form>
